...
function getStyleDir(compName) {
  ...
  if (!styleDir)
    styleDir = _chunk2VCLI5YNjs.kebabCase.call(void 0, compName);
  return styleDir;
}
...
function ElementPlusResolver(options = {}) {
  return (name) => {
    if (name.match(/^El[A-Z]/)) {
      const {
        version = _chunk2VCLI5YNjs.getPkgVersion.call(void 0, "element-plus", "1.0.2")
      } = options;
      const partialName = _chunk2VCLI5YNjs.kebabCase.call(void 0, name.slice(2));
...
function ElementUiResolver(options = {}) {
  return (name) => {
    if (name.startsWith("El")) {
      ...
      const partialName = _chunk2VCLI5YNjs.kebabCase.call(void 0, compName);
      return {
...
function IduxResolver(options = {}) {
  return (name) => {
    if (name.match(/^Ix[A-Z]/)) {
      ...
      const compName = name.slice(2);
      const kebabCaseName = _chunk2VCLI5YNjs.kebabCase.call(void 0, compName);
  ...
  return (name) => {
    ...
    if (name.startsWith("Van")) {
      ...
      return {
        ...
        path: "vant/es",
        sideEffects: importStyle ? `vant/es/${_chunk2VCLI5YNjs.kebabCase.call(void 0, partialName)}/style` : void 0
      };
    }
  };
}
  ...
  return (name) => {
    ...
    if (name.startsWith("Var")) {
      ...
      if (importStyle || importCss) {
        if (importStyle === "less" || importLess)
          sideEffects.push(`@varlet/ui/es/${_chunk2VCLI5YNjs.kebabCase.call(void 0, partialName)}/style/less.js`);
        else
          sideEffects.push(`@varlet/ui/es/${_chunk2VCLI5YNjs.kebabCase.call(void 0, partialName)}/style`);
      }
      return {
...
function VeuiResolver(options) {
  ...
  if (!components3) {
    try {
      const componentsData = _chunkBNJB64XRjs.__require.call(void 0, `${alias}/components.json`);
...
var formatters = {
  "kebab-case": _chunk2VCLI5YNjs.kebabCase,
  "camelCase": _chunk2VCLI5YNjs.camelCase,
  "PascalCase": _chunk2VCLI5YNjs.pascalCase
};
var peerPaths = new Map();
function assertPeerPath(peerPath) {
  if (!peerPaths.has(peerPath)) {
    try {
      _chunk2VCLI5YNjs.resolveImportPath.call(void 0, peerPath);
...
function getCompDir(compName) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (compName.match(matcher.pattern)) {
      compPath = `${matcher.compDir}/${_chunk2VCLI5YNjs.kebabCase.call(void 0, compName)}.vue`;
      break;
    }
  }
  if (!compPath)
    compPath = _chunk2VCLI5YNjs.kebabCase.call(void 0, compName);
  return compPath;
}
...
function VueUseComponentsResolver() {
  return (name) => {
    if (!components4) {
      try {
        const indexesJson = _chunkBNJB64XRjs.__require.call(void 0, "@vueuse/core/indexes.json");
...
exports.AntDesignVueResolver = AntDesignVueResolver; exports.ElementPlusResolver = ElementPlusResolver; exports.ElementUiResolver = ElementUiResolver; exports.HeadlessUiResolver = HeadlessUiResolver; exports.IduxResolver = IduxResolver; exports.LibraryResolver = _chunk2VCLI5YNjs.LibraryResolver; exports.NaiveUiResolver = NaiveUiResolver; exports.PrimeVueResolver = PrimeVueResolver; exports.VantResolver = VantResolver; exports.VarletUIResolver = VarletUIResolver; exports.VeuiResolver = VeuiResolver; exports.ViewUiResolver = ViewUiResolver; exports.VueUseComponentsResolver = VueUseComponentsResolver; exports.VuetifyResolver = VuetifyResolver; exports.tryLoadVeturTags = _chunk2VCLI5YNjs.tryLoadVeturTags;
