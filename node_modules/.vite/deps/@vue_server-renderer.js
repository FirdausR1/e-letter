...
function handleError2(err, instance, type, throwInDev = true) {
  ...
  if (instance) {
    let cur = instance.parent;
    const exposedInstance = instance.proxy;
    const errorInfo = true ? ErrorTypeStrings2[type] : type;
    while (cur) {
      const errorCapturedHooks = cur.ec;
      if (errorCapturedHooks) {
        for (let i = 0; i < errorCapturedHooks.length; i++) {
...
function logError2(err, type, contextVNode, throwInDev = true) {
  if (true) {
    const info = ErrorTypeStrings2[type];
    if (contextVNode) {
...
var globalCurrentInstanceSetters2;
var settersKey2 = "__VUE_INSTANCE_SETTERS__";
{
